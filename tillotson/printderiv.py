...
# Load derivatives
data2 = loadtxt('lookup.u1.txt')
data3 = loadtxt('lookup.udv2.txt')
data4 = loadtxt('lookup.u1dv2.txt')
data5 = loadtxt('testsplint.txt')
...
#		plot(data2[:,0],data2[:,i],'-',color='green',markersize=1,label='du/drho')

for i in range(1,size(data5[0,:]),1):
		plot(data5[:,0],data5[:,i],'-',color='orange',markersize=1,label='d2u/dv2')
