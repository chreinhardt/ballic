...
#include "till_isentropic_debug.h"
...
void msrInitialize(MSR *pmsr,MDL mdl,int argc,char **argv)
{
	...
	*pmsr = msr;
	...
	msr->param.bGasCondensed = 0;
	prmAddParam(msr->prm,"bGasCondensed",0,&msr->param.bGasCondensed,
				sizeof(int),"GasCondensed",
				"<Gas is Condensed> = +GasCondensed");
	...
	prmAddParam(msr->prm,"ddHonHLimit",2,&msr->param.ddHonHLimit,
				sizeof(double),"dhonh",
				"<|dH|/H Limiter> = 0.1");
	/* Set maximal smoothing length. */
	msr->param.dhMax = FLOAT_MAXVAL;
	prmAddParam(msr->prm,"dhMax",2,&msr->param.dhMax,
				sizeof(double),"dhMax",
				"<H Limiter> = inf");
	/* (CR) Print debug information. */
	fprintf(stderr,"(CR) msrInitialize: dhMax: %g\n",msr->param.dhMax);
	msr->param.dVelocityDamper = 0.0;
	prmAddParam(msr->prm,"dVelocityDamper",2,&msr->param.dVelocityDamper,
		    sizeof(double),"dVelocityDamper",
		    "Lose 0.0 dt velocity per step (no damping)");
	/*
	** Condensed gas phase after the Tillotson eq. of state. Default values are those for Granite given in Benz, Slattery and Cameron 1986.
	*/
	msr->param.dCond.a = 0.5;
	msr->param.dCond.b = 1.3;
	msr->param.dCond.u0 = 1.6e11; /* in ergs/g */
	msr->param.dCond.rho0 = 2.7; /* g/cc */
	msr->param.dCond.A = 1.8e11; /* ergs/cc */
	msr->param.dCond.B = 1.8e11; /* ergs/cc */
	msr->param.dCond.us = 3.5e10; /* ergs/g */
	msr->param.dCond.us2 = 1.8e11; /* ergs/g */
	msr->param.dCond.alpha = 5.0;
	msr->param.dCond.beta = 5.0;
	prmAddParam(msr->prm,"dCond_a",2,&msr->param.dCond.a,sizeof(double),
	    NULL,"<Condensed Equation of State parameter a> = 0.5");
	prmAddParam(msr->prm,"dCond_b",2,&msr->param.dCond.b,sizeof(double),
	    NULL,"<Condensed Equation of State parameter b> = 1.3");
	prmAddParam(msr->prm,"dCond_u0",2,&msr->param.dCond.u0,sizeof(double),
	    NULL,"<Condensed Equation of State parameter u0> = 1.6e11 ergs/g");
	prmAddParam(msr->prm,"dCond_rho0",2,&msr->param.dCond.rho0,sizeof(double),
	    NULL,"<Condensed Equation of State parameter rho0> = 2.7 g/cc");
	prmAddParam(msr->prm,"dCond_A",2,&msr->param.dCond.A,sizeof(double),
	    NULL,"<Condensed Equation of State parameter A> = 1.8e11 ergs/cc");
	prmAddParam(msr->prm,"dCond_B",2,&msr->param.dCond.B,sizeof(double),
	    NULL,"<Condensed Equation of State parameter B> = 1.8e11 ergs/cc");
	prmAddParam(msr->prm,"dCond_us",2,&msr->param.dCond.us,sizeof(double),
	    NULL,"<Condensed Equation of State parameter us> = 3.5e10 erg/g");
	prmAddParam(msr->prm,"dCond_us2",2,&msr->param.dCond.us2,sizeof(double),
	    NULL,"<Condensed Equation of State parameter us2> = 1.8e11 erg/g");
	prmAddParam(msr->prm,"dCond_alpha",2,&msr->param.dCond.alpha,sizeof(double),
	    NULL,"<Condensed Equation of State parameter alpha> = 5.0");
	prmAddParam(msr->prm,"dCond_beta",2,&msr->param.dCond.beta,sizeof(double),
	    NULL,"<Condensed Equation of State parameter beta> = 5.0");
...
#define msrSetGasModel( iModel ) { \
  if (msr->param.iGasModel == GASMODEL_UNSET) msr->param.iGasModel = iModel; \
	...
	if (msr->param.bGasCondensed      ) msrSetGasModel( GASMODEL_CONDENSED );
	...
	case GASMODEL_COOLING:
		...
		break;
	case GASMODEL_CONDENSED:
		msr->param.bGasCondensed = 1;
		break;
		}
...
void msrLogParams(MSR msr,FILE *fp)
{
	...
	fprintf(fp," GASOLINE");
#endif
#ifdef CONDENSED_DENSITY
	fprintf(fp," CONDENSED_DENSITY");
#endif
#ifdef ISENTROPIC
	fprintf(fp," ISENTROPIC");
#endif
#ifdef ISENTROPIC_DEBUG
	fprintf(fp," ISENTROPIC_DEBUG");
#endif
#ifdef TILL_ISENTROPIC_DEBUG_PARTICLE
	fprintf(fp," TILL_ISENTROPIC_DEBUG_PARTICLE");
#endif
#ifdef TILL_PRES_NONEG
	fprintf(fp," TILL_PRES_NONEG");
#endif
#ifdef QUINTICSPLINE
	fprintf(fp," QUINTICSPLINE");
#endif
#ifdef WENDLANDC2
	fprintf(fp," WENDLANDC2");
#endif
#ifdef WENDLANDC4
	fprintf(fp," WENDLANDC4");
#endif
#ifdef WENDLANDC6
	fprintf(fp," WENDLANDC6");
#endif
/* This macro does not work a will probably be removed soon. */
#ifdef VFAC
	fprintf(fp," VFAC");
#endif
#ifdef STARFORM
	...
	/* Log output for the Tillotson EOS */
	fprintf(fp," bGasCondensed: %d",msr->param.bGasCondensed);	
...
double msrReadTipsy(MSR msr)
{
...
#if defined(GASOLINE) && !defined(TEMP_IS_U)
	/* Changed 1.2.12 */
	/* Use u rather than T for condensed states */
	if (msr->param.iGasModel != GASMODEL_CONDENSED)
	...
	pstSetParticleTypes(msr->pst, NULL, 0, NULL, NULL);
...
void msrDrift(MSR msr,double dTime,double dDelta)
{
	...
	if (msr->param.bCannonical) {
		invpr.dvFacOne = 1.0 - fabs(dDelta)*msr->param.dVelocityDamper; /* Damp velocities (if desired) */
...
void msrKickDKD(MSR msr,double dTime,double dDelta)
{
	...
	if (msr->param.bCannonical) {
		in.dvFacOne = 1.0 - fabs(dDelta)*msr->param.dVelocityDamper; /* Damp velocities (if desired) */
#ifdef NEED_VPRED
		in.dvPredFacOne = 1.0 - fabs(dDelta)*msr->param.dVelocityDamper; /* Damp velocities (if desired) */
...
void msrBallMax(MSR msr, int iRung, int bGreater)
{
	...
	in.dhMax = msr->param.dhMax;
	/* (CR) Print debug information. */
	fprintf(stderr,"(CR) msrBallMax: dhMax: %g\n",msr->param.dhMax);
				   ...
				   int *piSec)
{
	...
	fprintf(stderr,"(CR) Starting msrTopStepKDK() \n");
    if(iAdjust && (iRung < msrMaxRung(msr)-1)) {
		if (msr->param.bVDetails) printf("Adjust, iRung: %d\n",iRung);
		msrActiveRung(msr, iRung, 1);
		msrActiveType(msr,TYPE_ALL,TYPE_TREEACTIVE|TYPE_SMOOTHACTIVE);
	...
	** (CR)Opening kick.
	*/
	fprintf(stderr,"(CR) Opening kick...\n");
    ...
    if (msrCurrMaxRungInclDF(msr) > iRung) {
		...
		*/
#ifdef ISENTROPIC
		/* Save the density before we drift the particles. */
		fprintf(stderr,"(CR) msrSaveDensity()\n");
		msrSaveDensity(msr);
#endif
		msrDrift(msr,dTime,0.5*dDelta);
		dTime += 0.5*dDelta;
			...
			*/	
			...
			if(msrDoGas(msr) && msrSphCurrRung(msr,iKickRung,1)) {
				...
				if (msr->param.bFastGas && msr->nActive < msr->nGas*msr->param.dFracFastGas) {
					/*
					** (CR) Smooth using fast gas. We have not implemented CONDENSED_DENSITY for fast gas yet
...
#ifdef CONDENSED_DENSITY
					msrSmooth(msr,dTime,SMX_MARKDENSITY,0);
#else
					msrSmooth(msr,dTime,SMX_MARKDENSITY,1);
#endif			
					/*
					** Smooth is done. We now have rhoold and rhonew at the updated
					** positions and can drift u.
					*/
#ifdef ISENTROPIC
					/* Evolve u along an isentrope. */
					fprintf(stderr,"(CR) msrDriftu() ...\n");
					msrDriftu(msr);
					/* Kick upred. */
					fprintf(stderr,"(CR) msrKickuPred() ...\n");
					msrKickuPred(msr,0.5*dDelta);
#endif
					if (msr->param.bVDetails)
						printf("Neighbours: %d ",
				...
				/*
				** (CR) Calculate pressure and sound speed from the EOS. We need to
				** do the drift and get upred before we can do this.
				*/	
				msrGetGasPressure(msr);
				...
				if (msr->param.bShockTracker) { 
			        ...
			        if (msr->param.bBulkViscosity) 
						...
						/*
						** (CR) Calculate SPH pressure forces.
						*/
 						fprintf(stderr,"SPH pressure forces...\n");
						msrReSmooth(msr,dTime,SMX_SPHPRESSURETERMS,1); 
			     }
	...
	** (CR) Closing kick.
	*/
	fprintf(stderr,"(CR) Closing kick...\n");
...
#ifdef GASOLINE
#ifdef ISENTROPIC
void msrSaveDensity(MSR msr)
{
	pstSaveDensity(msr->pst,NULL,0,NULL,NULL);
}
void msrDriftu(MSR msr)
{
	pstDriftu(msr->pst,NULL,0,NULL,NULL);
}
void msrKickuPred(MSR msr,double duDelta)
{
	struct inKickuPred in;
  
	in.duDelta =  duDelta;

	pstKickuPred(msr->pst,&in,sizeof(in),NULL,NULL);
}
#endif
...
void msrGetGasPressure(MSR msr)
{
	...
	case GASMODEL_COOLING:
		...
		break;
	case GASMODEL_CONDENSED:
	    in.dCond = msr->param.dCond; /* this sets all 10 parameters */
	    /*
	    ** Convert energies and densities to code units!
	    */
	    in.dCond.u0 /= msr->param.dErgPerGmUnit;
	    in.dCond.us /= msr->param.dErgPerGmUnit;
	    in.dCond.us2 /= msr->param.dErgPerGmUnit;
	    in.dCond.rho0 /= msr->param.dGmPerCcUnit;
	    in.dCond.A /= (msr->param.dGmPerCcUnit*msr->param.dErgPerGmUnit);
	    in.dCond.B /= (msr->param.dGmPerCcUnit*msr->param.dErgPerGmUnit);
	    break;
	case GASMODEL_GLASS:
#ifdef GLASS
...
void msrInitSph(MSR msr,double dTime)
{
	...
	/*
	** (CR) We have to set msrBallMax() before we do the smooth to make sure
	** that dhMax is properly set.
	*/

#ifdef ISENTROPIC
	/* (CR) Print debug information. */
	fprintf(stderr,"**************************************\n");
	fprintf(stderr,"(CR) msrInitSph: msrBallMax(msr, 0, 1)\n");
	fprintf(stderr,"**************************************\n");
#endif
